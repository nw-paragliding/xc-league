      ...
      <div className="page-header">
        <div>
          ...
          <div className="page-subtitle">
            {season ? `${season.name} · ${taskCount} task${taskCount !== 1 ? 's' : ''}` : 'Loading…'}
          </div>
        </div>
      </div>
            ...
            {standings.slice(0, 3).map((p, i) => (
              <div
                ...
                style={{
                  flex: 1,
                  background:
                    i === 0 ? 'linear-gradient(135deg, rgba(232,168,66,0.12), rgba(232,168,66,0.04))' : 'var(--bg2)',
              ...
              >
                ...
                >
                  {p.pilotName}
                  {p.pilotId === user?.id && (
                    <span style={{ marginLeft: 6, fontSize: 10, color: 'var(--gold)', fontFamily: 'var(--font-mono)' }}>
                      you
                    </span>
                  )}
                </div>
              ...
              {isLoading
                ...
                : standings.map((row) => (
                    <tr key={row.pilotId} className={`lb-row${row.pilotId === user?.id ? ' me' : ''}`}>
                      ...
                      <td className="right">
                        <span className="mono" style={{ fontSize: 12, color: 'var(--sky)' }}>
                          {row.tasksWithGoal}
                        </span>
                      </td>
                      <td className="right">
