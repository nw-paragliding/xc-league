...
export const leagueApi = {
  ...
  list: () => api.get<{ leagues: League[] }>('/leagues'),

  /** Create a new league (any authenticated user) */
  create: (data: CreateLeagueInput) => api.post<{ league: League }>('/leagues', data),

  /** Update league details (league admin only) */
  update: (leagueSlug: string, data: UpdateLeagueInput) => api.put<{ league: League }>(`/leagues/${leagueSlug}`, data),

  /** Join a league as a pilot */
  join: (leagueSlug: string) => api.post<{ message: string }>(`/leagues/${leagueSlug}/join`),

  // ── Member Management ──────────────────────────────────────────────────

  /** List all members of a league */
  listMembers: (leagueSlug: string) => api.get<{ members: LeagueMember[] }>(`/leagues/${leagueSlug}/members`),
  ...
  listSeasons: (leagueSlug: string) => api.get<{ seasons: Season[] }>(`/leagues/${leagueSlug}/seasons`),
  ...
  getSeasonRegistration: (leagueSlug: string, seasonId: string) =>
    api.get<{ isRegistered: boolean; registrationCount: number }>(
      `/leagues/${leagueSlug}/seasons/${seasonId}/registration`,
    ),

  /** List pilots registered for season (admin only) */
  listSeasonRegistrations: (leagueSlug: string, seasonId: string) =>
    api.get<{ pilots: Array<{ userId: string; email: string; displayName: string; registeredAt: string }> }>(
      `/leagues/${leagueSlug}/seasons/${seasonId}/registrations`,
    ),
  ...
  getTaskQrUrl: (
    ...
    seasonId: string,
    taskId: string,
    app: 'xctrack' | 'download',
    format: 'xctsk' | 'cup' = 'xctsk',
  ): string => `/api/v1/leagues/${leagueSlug}/seasons/${seasonId}/tasks/${taskId}/qr?app=${app}&format=${format}`,
  ...
  importTask: (
    ...
    return apiFetch(`/leagues/${leagueSlug}/seasons/${seasonId}/tasks/import${qs}`, { method: 'POST', body: form });
  },

  /** Preview all tasks in a .cup file without creating anything */
  cupPreview: (leagueSlug: string, seasonId: string, file: File): Promise<{ tasks: CupPreviewTask[] }> => {
    const form = new FormData();
    form.append('file', file, file.name);
    return apiFetch(`/leagues/${leagueSlug}/seasons/${seasonId}/tasks/cup-preview`, { method: 'POST', body: form });
  },
  ...
  bulkImport: (
    ...
    return apiFetch(`/leagues/${leagueSlug}/seasons/${seasonId}/tasks/bulk-import`, { method: 'POST', body: form });
  },
};
