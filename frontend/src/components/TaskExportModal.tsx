...
export default function TaskExportModal({ task, leagueSlug, onClose }: Props) {
  ...
  const qrSrc = seasonId ? leagueApi.getTaskQrUrl(leagueSlug, seasonId, task.id, 'xctrack', 'xctsk') : null;
  ...
  const handleDownload = async (format: 'xctsk' | 'cup') => {
    if (!seasonId) {
      setDownloadError('Season ID unavailable — please refresh and try again.');
      return;
    }
      ...
      >
        ...
        >
          <div>
            ...
            <div style={{ fontSize: 16, fontWeight: 700, color: 'var(--text)' }}>{task.name}</div>
          </div>
          <button
            onClick={onClose}
            style={{
