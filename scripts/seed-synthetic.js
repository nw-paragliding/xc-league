...
const PILOTS = [
  ...
  { name: 'Riley Park',       email: 'riley.park@example.com',      skill: 0.50 },
...
function computeScore(task, skill, j) {
  ...
  const distKm        = reachedGoal
    ? task.distKm
    : task.distKm * (0.3 + (skill + j) * 0.65);
...
function jitter(pilotIdx, taskIdx) {
  ...
  return (x - Math.floor(x)) * 0.30 - 0.15;
}
...
db.transaction(() => {
  ...
  for (let pi = 0; pi < PILOTS.length; pi++) {
    ...
    for (let ti = 0; ti < TASKS.length; ti++) {
      ...
      const { reachedGoal, distKm, distFraction, distancePoints, timePoints, taskTimeS } =
        computeScore(task, pilot.skill, j);

      const submissionId = randomUUID();
      const attemptId    = randomUUID();
      const igcText      = makeIgc(pilot.name, pi, ti, distFraction);
      const sha          = Buffer.from(pilot.email + task.id).toString('hex').slice(0, 64).padEnd(64, '0');

      insertSubmission.run(
        submissionId, task.id, uid, LEAGUE_ID,
        Buffer.from(igcText, 'utf8'),
        `${pilot.name.replace(/ /g, '_')}_${task.name}.igc`,
        igcText.length, sha, attemptId,
      );

      insertAttempt.run(
        attemptId, submissionId, task.id, uid, LEAGUE_ID,
        ...
        distKm,
        distancePoints, timePoints, distancePoints + timePoints,
      );
      ...
      `).run(
        randomUUID(), task.id, uid, LEAGUE_ID, attemptId,
        distKm, reachedGoal ? 1 : 0, taskTimeS,
        distancePoints, timePoints, distancePoints + timePoints,
      );
    }
  }
  ...
  for (const task of TASKS) {
    const rows = db.prepare(
      `SELECT id, total_points FROM task_results WHERE task_id = ? ORDER BY total_points DESC`
    ).all(task.id);
    rows.forEach((row, i) => {
  ...
  standingRows.forEach((row, i) => {
    upsertStanding.run(
      randomUUID(), SEASON_ID, row.user_id, LEAGUE_ID,
      row.total_points, row.tasks_flown, row.tasks_with_goal, i + 1,
    );
  });

  console.log(`Done — inserted ${PILOTS.length} pilots across ${TASKS.length} tasks, ${standingRows.length} in standings.`);
})();
