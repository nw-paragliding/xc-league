...
function closestOnSegment(p1: Vec2, p2: Vec2, q: Vec2): Vec2 {
  ...
  const t = Math.max(0, Math.min(1,
    ((q.x - p1.x) * seg.x + (q.y - p1.y) * seg.y) / len2,
  ));
...
export function optimiseRoute(cylinders: Cylinder[]): OptimisedRoute {
  ...
  pts[n - 1] = goalIsLine
    ? { ...centres[n - 1] }
    : pointToward(centres[n - 1], radii[n - 1], centres[n - 2]);
  ...
  const finalGoalLineBearing = goalIsLine
    ? goalLineBearing(inboundBearing(pts[n - 2], centres[n - 1]))
    : 0;
...
export function computePartialDistanceKm(
  ...
  const completedKm = route.legDistancesKm
    .slice(0, lastTpIdx)
    .reduce((s, d) => s + d, 0);
  ...
  for (const fix of pilotFixes) {
    ...
    const dot = (fixLocal.x - lastLocal.x) * legDir.x
              + (fixLocal.y - lastLocal.y) * legDir.y;
