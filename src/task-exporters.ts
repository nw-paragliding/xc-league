...
export function exportXctsk(task: ExportTask): string {
  ...
  const turnpointsXml = tps.map((tp, i) => {
...
export function exportCup(task: ExportTask): string {
  ...
  const waypointLines = tps.map(tp =>
...
export function buildDownloadUrl(baseUrl: string, leagueSlug: string, seasonId: string, taskId: string, format: 'xctsk' | 'cup'): string {
