...
export async function registerAdminRoutes(fastify: FastifyInstance, opts: AdminRouteOptions): Promise<void> {
  const { db } = opts;
  ...
  fastify.post('/admin/users/:userId/promote', async (request, reply) => {
    ...
    const targetUser = db
      .prepare(`SELECT id, email, is_super_admin FROM users WHERE id = ? AND deleted_at IS NULL`)
    ...
    // Promote user
    db.prepare(`UPDATE users SET is_super_admin = 1, updated_at = datetime('now') WHERE id = ?`).run(userId);
  ...
  fastify.post('/admin/users/:userId/demote', async (request, reply) => {
    ...
    const superAdminCount = db
      .prepare(`SELECT COUNT(*) as count FROM users WHERE is_super_admin = 1 AND deleted_at IS NULL`)
      .get() as { count: number };
    ...
    // Demote user
    db.prepare(`UPDATE users SET is_super_admin = 0, updated_at = datetime('now') WHERE id = ?`).run(userId);
  ...
  fastify.delete('/admin/leagues/:leagueSlug', async (request, reply) => {
    ...
    const league = db.prepare(`SELECT id, name FROM leagues WHERE slug = ? AND deleted_at IS NULL`).get(leagueSlug) as
    ...
    db.prepare(`UPDATE leagues SET deleted_at = datetime('now'), updated_at = datetime('now') WHERE id = ?`).run(
      league.id,
    );
