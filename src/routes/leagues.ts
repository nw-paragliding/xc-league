...
import {
  buildDownloadUrl,
  buildXctrackDeepLink,
  type ExportTask,
  type ExportTurnpoint,
  exportCup,
  exportXctsk,
} from '../task-exporters';
import type { ParsedTurnpoint } from '../task-parsers';
import { parseCupAll, parseTaskFile } from '../task-parsers';
...
export async function registerLeagueRoutes(fastify: FastifyInstance, opts: LeagueRouteOptions): Promise<void> {
  ...
  fastify.post('/leagues', async (request, reply) => {
    ...
    const existing = db.prepare(`SELECT id FROM leagues WHERE slug = ? AND deleted_at IS NULL`).get(body.slug);
  ...
  await fastify.register(async (leagueScope) => {
    ...
    leagueScope.get('/leagues/:leagueSlug/seasons/:seasonId/tasks', async (request, reply) => {
      ...
      const season = db
        .prepare(`SELECT id FROM seasons WHERE id = ? AND league_id = ? AND deleted_at IS NULL`)
    ...
    leagueScope.get(
      ...
      async (request, reply) => {
        ...
        const igcText =
          submission.igcData instanceof Buffer ? submission.igcData.toString('utf8') : String(submission.igcData);
        ...
        return reply.send({
          ...
          crossings: crossings.map((c: any) => ({
            ...
            crossingTimeMs:
              typeof c.crossingTimeMs === 'string' ? new Date(c.crossingTimeMs).getTime() : c.crossingTimeMs,
            type: c.type,
            radiusM: c.radiusM,
            latitude: c.latitude,
          ...
          meta: {
            ...
            durationS:
              fixes.length >= 2 ? Math.round((fixes[fixes.length - 1].timestamp - fixes[0].timestamp) / 1000) : null,
    ...
    leagueScope.get('/leagues/:leagueSlug/seasons/:seasonId/standings', async (request, reply) => {
      ...
      const season = db
        ...
        .get(seasonId, league.id) as
        | { id: string; name: string; competitionType: string; startDate: string; endDate: string; taskCount: number }
        | undefined;
    ...
    leagueScope.post('/leagues/:leagueSlug/seasons/:seasonId/tasks/import', async (request, reply) => {
      ...
      db.transaction(() => {
        ...
        if (sssId || essId || goalId) {
          ...
          if (sssId) {
            setClauses.push('sss_turnpoint_id = ?');
            setParams.push(sssId);
          }
          if (essId) {
            setClauses.push('ess_turnpoint_id = ?');
            setParams.push(essId);
          }
          if (goalId) {
            setClauses.push('goal_turnpoint_id = ?');
            setParams.push(goalId);
          }

          db.prepare(`UPDATE tasks SET ${setClauses.join(', ')} WHERE id = ?`).run(...setParams, taskId);
        }
      })();
    ...
    leagueScope.post('/leagues/:leagueSlug/seasons/:seasonId/tasks/bulk-import', async (request, reply) => {
      ...
      for await (const part of request.parts()) {
        if (part.type === 'file' && part.fieldname === 'file') {
          ...
          try {
            tasksConfig = JSON.parse(part.value as string);
          } catch {}
        }
      }
      ...
      db.transaction(() => {
        for (const cfg of tasksConfig) {
          ...
          if (sssId || essId || goalId) {
            ...
            if (sssId) {
              sets.push('sss_turnpoint_id = ?');
              vals.push(sssId);
            }
            if (essId) {
              sets.push('ess_turnpoint_id = ?');
              vals.push(essId);
            }
            if (goalId) {
              sets.push('goal_turnpoint_id = ?');
              vals.push(goalId);
            }
    ...
    leagueScope.post('/leagues/:leagueSlug/seasons/:seasonId/tasks/:taskId/publish', async (request, reply) => {
      ...
      const turnpointCount = db.prepare(`SELECT COUNT(*) as count FROM turnpoints WHERE task_id = ?`).get(taskId) as {
        count: number;
      };
    ...
    leagueScope.post('/leagues/:leagueSlug/seasons/:seasonId/tasks/:taskId/unpublish', async (request, reply) => {
      ...
      const submissionCount = db
        .prepare(`SELECT COUNT(*) as count FROM flight_submissions WHERE task_id = ? AND deleted_at IS NULL`)
    ...
    leagueScope.put('/leagues/:leagueSlug/seasons/:seasonId/tasks/reorder', async (request, reply) => {
      ...
      const existing = db
        .prepare(`SELECT id FROM tasks WHERE id IN (${placeholders}) AND season_id = ? AND deleted_at IS NULL`)
    ...
    leagueScope.get('/leagues/:leagueSlug/seasons/:seasonId/tasks/:taskId/download', async (request, reply) => {
      ...
      const exportTask: ExportTask = {
        ...
        turnpoints: tpRows,
        rawContent: taskRow.rawContent,
        dataSource: taskRow.dataSource,
      };
      ...
      if (format === 'xctsk') {
        ...
        fileContent = taskRow.dataSource === 'cup' && taskRow.rawContent ? taskRow.rawContent : exportCup(exportTask);
    ...
    leagueScope.put('/leagues/:leagueSlug', async (request, reply) => {
      ...
      if (body.slug !== undefined) {
        ...
        const existing = db
          .prepare(`SELECT id FROM leagues WHERE slug = ? AND id != ? AND deleted_at IS NULL`)
