...
declare class Buffer {
  static from(b: ArrayBuffer): { toString(enc: string): string };
}
...
type FastifyPluginAsync<O = unknown> = (
  app: {
...
export async function verifyJwt(token: string, config: AuthConfig): Promise<JwtClaims | null> {
  try {
...
export async function exchangeGoogleCode(code: string, config: AuthConfig): Promise<GoogleUserInfo> {
 ...
 */
export function findOrCreateGoogleUser(googleUser: GoogleUserInfo, db: any): UserRecord {
  ...
  if (existingIdentity) {
    ...
    const user = db.prepare(`SELECT * FROM users WHERE id = ?`).get(existingIdentity.user_id) as UserRecord | undefined;
  ...
  return db.transaction((): UserRecord => {
    ...
    const existingUser = db.prepare(`SELECT * FROM users WHERE email = ?`).get(googleUser.email) as
      | UserRecord
      | undefined;
 ...
 * with stateless JWTs.
 */
const authPluginImpl: any = async (fastify: any, { config, db }: { config: AuthConfig; db: Database }) => {
  ...
  fastify.addHook('preHandler', async (request: any, _reply: any) => {
    ...
    if (process.env['NODE_ENV'] === 'test' && process.env['ENABLE_TEST_AUTH'] === 'true') {
      ...
      if (testUserId) {
        const userRow = db
          .prepare(`SELECT id, is_super_admin, token_version FROM users WHERE id = ? AND deleted_at IS NULL`)
    ...
    const userRow = db
      .prepare(`SELECT token_version, is_super_admin FROM users WHERE id = ? AND deleted_at IS NULL`)
 ...
 */
export async function handleGoogleAuthInitiate(request: any, reply: any, config: AuthConfig): Promise<void> {
 ...
 */
export async function handleUpdateMe(request: any, reply: any, db: any): Promise<void> {
  requireAuth(request, reply);
  ...
  if (body.gliderManufacturer !== undefined) {
    db.prepare(`UPDATE users SET glider_manufacturer = ?, updated_at = datetime('now') WHERE id = ?`).run(
      body.gliderManufacturer,
      request.user!.userId,
    );
  }
  if (body.gliderModel !== undefined) {
    db.prepare(`UPDATE users SET glider_model = ?, updated_at = datetime('now') WHERE id = ?`).run(
      body.gliderModel,
      request.user!.userId,
    );
  }
  if (body.gliderWeightRating !== undefined) {
    if (
      body.gliderWeightRating !== null &&
      (typeof body.gliderWeightRating !== 'number' || body.gliderWeightRating <= 0)
    ) {
      reply.status(400).send({ error: 'Invalid weight rating. Must be a positive number (kg).' });
      return;
    }
    db.prepare(`UPDATE users SET glider_weight_rating = ?, updated_at = datetime('now') WHERE id = ?`).run(
      body.gliderWeightRating,
      request.user!.userId,
    );
  }
 ...
 */
export async function handleLogout(request: any, reply: any, config: AuthConfig): Promise<void> {
 ...
 */
export async function handleRevokeTokens(request: any, reply: any, db: any): Promise<void> {
  requireAuth(request, reply);
  ...
  if (body.userId && body.userId !== targetUserId) {
    ...
    if (!request.user!.isAdmin) {
      reply.status(403).send({ error: "Cannot revoke another user's tokens" });
      return;
    }
